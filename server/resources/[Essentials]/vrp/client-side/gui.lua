...
RegisterCommand("keybind",function(source,args)
	if not IsPauseMenuActive() then
		...
		if not celular and GetEntityHealth(ped) > 101 then
			...
			elseif args[1] == "f3" and not IsPedInAnyVehicle(ped) then
			...
			elseif args[1] == "f10" and not IsPedInAnyVehicle(ped) then
...
RegisterKeyMapping("keybind f3","Animation Bind 6","keyboard","f3")
RegisterKeyMapping("keybind 7","Animation Bind 7","keyboard","7")
RegisterKeyMapping("keybind 8","Animation Bind 8","keyboard","8")
RegisterKeyMapping("keybind f10","Animation Bind 9","keyboard","f10")

...
vRP.prepare("vRP/get_invoice","SELECT * FROM smartphone_bank_invoices WHERE payer_id = @user_id")

--vRP.prepare("vRP/get_myinvoice","SELECT * FROM vrp_invoice WHERE nuser_id = @nuser_id")
vRP.prepare("vRP/get_myinvoice","SELECT * FROM smartphone_bank_invoices WHERE payee_id = @nuser_id")

--vRP.prepare("vRP/del_invoice","DELETE FROM vrp_invoice WHERE id = @id AND user_id = @user_id")
vRP.prepare("vRP/del_invoice","DELETE FROM smartphone_bank_invoices WHERE id = @id AND payer_id = @user_id")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PREPARE vRP_SALARY
-----------------------------------------------------------------------------------------------------------------------------------------
...
vRP.prepare("vRP/set_rental_time","UPDATE vrp_vehicles SET premiumtime = @premiumtime + 2592000 WHERE user_id = @user_id AND vehicle = @vehicle")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PREPARE vRP_CHESTS
-----------------------------------------------------------------------------------------------------------------------------------------

...
$(document).ready(function() {
    document.onkeydown = function(data) {
        if (data.keyCode == 27) {
            ...
            $('#total').html('0'); 
            change = {};
            $.post('http://vrp_tattoo/reset', JSON.stringify({}))           
        }
    }

     $("#leftHeading").click(function() {
        $.post('http://vrp_tattoo/leftHeading', JSON.stringify({ value: 10 }));
    })

    $("#handsUp").click(function() {
        $.post('http://vrp_tattoo/handsUp', JSON.stringify({}));
    })

    $("#rightHeading").click(function() {
        $.post('http://vrp_tattoo/rightHeading', JSON.stringify({ value: 10 }));
    })

    $("#payament").click(function() {
        $(".loja-de-tattoo").fadeOut()
        $.post('http://vrp_tattoo/payament', JSON.stringify({ price: $('#total').text() }));
        $('#total').html('0');
        applyTattoo = null;
        tattooShop = null
    }) 
                        ...
                        let price = tattooShop[atualPart].tattoo[i].price;
                            $("footer").append(`
                                <div class="item-tattoo" data-partname="${partName}" data-partid="${i}" data-parttype="${atualPart}" onclick="select(this)" id="${atualPart}${i}">
                                    <div class="img-tattoo" style="background-repeat: no-repeat;background-size:100%;background-image: url('https://itens.evolutionfivem.com/tattoo/${partName}.jpg')">  
                                        <div class="overlay">
...
function select(element) {
    ...
    $.post('http://vrp_tattoo/changeTattoo', JSON.stringify({ type: partName, id: partId, type: partType }));    
}

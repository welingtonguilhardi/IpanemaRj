...
RegisterNetEvent("chamarcaixaeletronico")
AddEventHandler("chamarcaixaeletronico",function()
	local source = source
	inicio_cancelar(source)
end)
			...
			for k,v in pairs(locais) do
				if Vdist(v.x,v.y,v.z,x,y,z) <= 1 and not andamento then
						if vSERVER.checkItem() then
							if vSERVER.checkRobbery(v.x,v.y,v.z) then
								vSERVER.getItem()
								if v.id == 1 then
									t_roubo = 25
								...
								TriggerEvent("Notify","amarelo","Você plantou a bomba, cuidado...",5000)
								contador_id = 0		
								while contador_id <= t_roubo do
									local coords = GetEntityCoords(ped)
									if t_roubo == 25 then
										TriggerEvent("sounds:distance", coords, 0.8, 'bomb_25', 0.5)
										Wait(25000)
								...
								if not HasModelLoaded(dinheiro_prop) then
									...
									while contador >= 0 do 
										...
										elseif Vdist(v.x,v.y,v.z,x,y,z) > 15 then
											...
											TriggerEvent("Notify","vermelho","Você fugiu do roubo e deixou tudo para trás.",5000)
											contador = -1
										end
									end

...
vRP.prepare("vRP/get_chars","SELECT chars FROM vrp_infos WHERE steam = @steam")
-----------------------------------------------------------------------------------------------------------------------------------------
-- CREATECHAR
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("spawn:createChar",function(name,name2,sex)
	...
	local countchars = vRP.query("vRP/get_chars",{ steam = steam })
	-- print(countchars[1].chars)
	-- print(parseInt(#persons))

	if countchars[1].chars <= parseInt(#persons) then

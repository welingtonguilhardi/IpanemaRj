    ...
    while true do
        ...
        for k,v in pairs(homesList) do
            ...
            if distance <= 1.5 then
                timeDistance = 5
                DrawText3Ds(v[5],v[6],v[7],"~w~Mentalize F9")
            end
        end
...
RegisterNUICallback("takeItem",function(data)
	vSERVER.takeItem(tostring(houseOpen),data.item,data.slot,data.amount,data.custom)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- STOREITEM
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- PORTA
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("homes:porta")
AddEventHandler("homes:porta",function(source,args)
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	for k,v in pairs(homesList) do
		local distance = #(coords - vector3(v[5],v[6],v[7]))
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- Functions invade webhook
-----------------------------------------------------------------------------------------------------------------------------------------
function cRP.GetHome (source)
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	for k,v in pairs(homesList) do
		local distance = #(coords - vector3(v[5],v[6],v[7]))
...
function cRP.GetHomeCoords (source)
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	if ped then
		return coords
...
RegisterNetEvent("homes:visitar")
AddEventHandler("homes:visitar",function()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	for k,v in pairs(homesList) do
		--print("teste")
		...
		if distance <= 1.5 then
			...
			if v[1] == "vip01" then
				SetEntityCoords(ped,-2588.1, 1909.76, 167.51)
				table.insert(internHouses,{ -2588.1, 1909.76, 167.51,"exit","SAIR" })
				table.insert(internHouses,{ -2583.92, 1881.63, 167.32,"vault","ABRIR" })
			end
			if v[1] == "vip02" then
				SetEntityCoords(ped,-288.31, -722.39, 125.48)
				table.insert(internHouses,{ -288.31, -722.39, 125.48,"exit","SAIR" })
				table.insert(internHouses,{ -275.44, -724.23, 125.48,"vault","ABRIR" })
			end
			if v[1] == "vip03" then
				SetEntityCoords(ped,-1987.06, -503.72, 12.18)
				table.insert(internHouses,{ -1987.06, -503.72, 12.18,"exit","SAIR" })
				table.insert(internHouses,{ -1986.23, -498.1, 20.74,"vault","ABRIR" })
			end
			if v[1] == "vip04" then
				SetEntityCoords(ped,-3216.75, 816.54, 8.93)
				table.insert(internHouses,{ -3218.36, 815.24, 8.93,"exit","SAIR" })
	...
	Citizen.Wait(20000)
	print("tempo")
	SetEntityCoords(ped,coords.x,coords.y,coords.z,1,0,0) --20 segundos
...
-- Invadir
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("homes:invade")
AddEventHandler("homes:invade",function(source,args)
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	for k,v in pairs(homesList) do
		local distance = #(coords - vector3(v[5],v[6],v[7]))
...
function cRP.setBlipsOwner(x,y,z,homeName)

...
const updateDrag = () => {
	...
	$('.empty').droppable({
		...
		drop: function(event,ui){
			...
			itemData = { key: ui.draggable.data('item-key'), slot: ui.draggable.data('slot'), custom:  ui.draggable.data('custom') };
	...
	$.post("http://chest/requestChest",JSON.stringify({}),(data) => {
		...
		for (let x = 1; x <= 100; x++){
			...
			if (data.inventario[slot] !== undefined) {
				...
				const item = `<div class="item populated" title="" data-unity="${v["unity"]}" data-custom="${v.custom}" data-tipo="${v["tipo"]}" data-desc="${v["desc"] !== "undefined" ? "<br><description>"+v["desc"]+"</description>":""}"  data-economy="${v["economy"]}" data-max="${v["max"]}" data-type="${v["type"]}" data-serial="${v["serial"]}" style="background-image: url('nui://inventory/web-side/images/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-custom="${v.custom}" data-amount="${v.amount}" data-peso="${v.peso}" data-item-key="${v.key}" data-name-key="${v.name}" data-slot="${slot}"">
					<div class="top">

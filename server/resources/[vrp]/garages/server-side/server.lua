...
local garages = {
	...
	[19] = { ["name"] = "Paramedic", ["payment"] = true, ["perm"] = "nc.permissao" },
	[20] = { ["name"] = "Heliparamedic", ["payment"] = true, ["perm"] = "nc.permissao" },
	[21] = { ["name"] = "Police", ["payment"] = true, ["perm"] = "nc.permissao" },
	[22] = { ["name"] = "PoliceS", ["payment"] = true, ["perm"] = "nc.permissao" },
	-- [23] = { ["name"] = "Police", ["payment"] = true, ["perm"] = "nc.permissao" },
	[24] = { ["name"] = "Helipolice", ["payment"] = true, ["perm"] = "nc.permissao" },
	[25] = { ["name"] = "Helipolice", ["payment"] = true, ["perm"] = "nc.permissao" },
	[26] = { ["name"] = "Helipolice", ["payment"] = true, ["perm"] = "nc.permissao" },
...
--	[37] = { ["name"] = "Garage", ["payment"] = true, ["public"] = true },
	...
	[39] = { ["name"] = "Tow", ["payment"] = true, ["perm"] = "nc.permissao" },
	[40] = { ["name"] = "Tow", ["payment"] = true, ["perm"] = "nc.permissao" },
	[41] = { ["name"] = "Tow", ["payment"] = true, ["perm"] = "nc.permissao" },
...
--	[46] = { ["name"] = "Ilegal", ["payment"] = true, ["public"] = true },
	...
	[614] = { ["name"] = "Ballas", ["payment"] = false, ["perm"] = 'nc.permissao' },
...
--gps
--------------------------------------------------------------------------------------------------------------------------------------
function cnVRP.trygps(vehname)
	local source = source
	local user_id = vRP.getUserId(source)
	vRP.prepare("vRP/gps","SELECT * FROM vrp_vehicles WHERE user_id = @user_id AND vehicle = @vehicle AND gps = 1")
	local vehicle = vRP.query("vRP/gps",{ user_id = parseInt(user_id), vehicle = vehname })
	if vehicle[1] == nil then
		return false
	else	
		return vehicle[1].vehicle
	end	
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- SPAWNVEHICLES
...
function cnVRP.spawnVehicles(name,use)
    ...
    if user_id and name then
        if not vCLIENT.returnVehicle(source,name) then
			...
			-- elseif vRP.vehicleType(tostring(name)) == "donate" and vRP.getCarPremium2(name,user_id)then
			...
			-- 	if status then
			-- 		vRP.prepare("vRP/set_premiumm","UPDATE vrp_vehicles SET premiumtime = @premiumtime, predays = @predays WHERE user_id = user_id = @user_id AND vehicle = @vehicle")
			-- 		if vRP.remGmsId(user_id,parseInt(vRP.vehiclePrice(name))) then
			-- 			-- print(user_id)
			-- 			-- print(tostring(name))
			-- 			-- print(parseInt(os.time()))
			-- 			vRP.execute("vRP/set_premiumm",{ user_id = user_id, vehicle = name, premiumtime = parseInt(os.time()), predays = 3 } )
			-- 		else
			...
			-- 	end
				...
				if garages[use].payment and not vRP.getPremium2(parseInt(user_id)) then
...
-- delete
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("dellvehicle")
AddEventHandler("dellvehicle",function()
	local source = source
	local vehicle = vRPclient.getNearVehicle(source,15)
	if vehicle then
		vCLIENT.deleteVehicle(source,vehicle)
...
-- RegisterCommand("getgps",function(source,args,rawCommand)
...
-- 		else
-- 			TriggerClientEvent("Notify",source,"GPS","Veículo possui gps.",5000)
-- 		end	

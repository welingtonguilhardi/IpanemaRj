...
Citizen.CreateThread(function()
	...
	AddBoxZone("servicePolice01",vector3(-918.36,-2033.52,9.41),0.4,0.4,{
	...
	AddTargetModel({ 0x04430687 },{ -- Mec Service
	options = {
		{
	...
	AddTargetModel({ 0x2799EFD8 },{ -- Paramedic
	options = {
		{
	...
	AddTargetModel({ 0x625D6958 },{ -- Sul Service
		options = {
			{
	...
	AddTargetModel({ 2117668672,-1498379115,-1519439119,-289946279 },{
		options = {
			{
	...
	AddTargetModel({ 0x49EADBF6 },{
		options = {
			{
				event = "shops:restauranteStore",
				label = "Comprar",
				tunnel = "client"
			}
		},
...
--	})
	...
	AddTargetModel({ -1734476390 },{ -- fishing 
	options = {
		{
	...
	AddTargetModel({ 843005760 },{ -- Open benefactor
		options = {
			{
...
AddEventHandler("target:pacienteDeitar",function()
	if not exports["player"]:blockCommands() and not exports["player"]:handCuff() then
		...
		if GetEntityHealth(ped) > 101 then
			while GetEntityHealth(ped) < 200 do
				if IsEntityPlayingAnim(ped,"anim@gangops@morgue@table@","body_search",3)then
					if vSERVER.checkServices() then
						vRP.updateHealth(1)
					else
						--TriggerEvent("Notify","amarelo","Existem medicos em serviço,aguarde um medico te atender.",5000)
						Citizen.Wait(25000)
						break
					end		
				end

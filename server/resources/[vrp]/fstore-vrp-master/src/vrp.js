...
vrp.getName = async (id) => {
  ...
  const table = hasPlugin('name_in_vrp_users') ? 'vrp_users' : 'vrp_infos';
...
vrp.addCar = vrp.addVehicle = async (id, spawn, fields = {}) => {
  ...
  if (row) return new Warning('Este jogador já possui esse veículo');
    ...
    if (hasPlugin('@centralroleplay')) {
      ...
      const [udata] = await sql(`SELECT registration FROM vrp_infos WHERE id=?`, [id], true);
      data['steam'] = 'P ' + udata.registration;
    }

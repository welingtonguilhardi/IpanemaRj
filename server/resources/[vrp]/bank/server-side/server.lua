...
function cRP.requestInvoices()
	...
	if user_id then
		...
		for k,v in pairs(consult) do
			if v.nuser_id == 0 then
				table.insert(invoices,{ id = v.id, user_id = parseInt(v.user_id), nuser_id = parseInt(v.payer_id), name = "Governament", price = parseInt(v.value), text = tostring(v.reason) })
			else
				local identity = vRP.getUserIdentity(v.payee_id)
				table.insert(invoices,{ id = v.id, user_id = parseInt(v.user_id), nuser_id = parseInt(v.payee_id), name = tostring(identity.name.." "..identity.name2), price = parseInt(v.value), text = tostring(v.reason) })
			end
		end
		return invoices
	end
...
function cRP.requestMyInvoices()
	...
	if user_id then
		...
		for k,v in pairs(consult) do
			...
			if identity then
				table.insert(invoices,{ name = tostring(identity.name.." "..identity.name2), date = v.paid, price = parseInt(v.value), text = tostring(v.reason) })
			end
		end
		return invoices
	end
